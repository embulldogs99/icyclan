...
func dbpull1() []Newspoint {
  ...
  if err != nil {log.Fatalf("Unable to connect to the database")}
  sqlstatmt:="SELECT target,price,returns,ticker,note,date,q_eps,a_eps,report FROM fmi.marketmentions WHERE report='analyst' AND date > current_timestamp - INTERVAL '2 days';"
